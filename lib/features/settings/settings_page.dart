    ...
    return ListView(
      ...
      children: [
        Padding(
          ...
          child: const SeraphHeader(
            title: 'Seraph',
            accent: 'Settings',
            subtitle: 'Konfigurasi koneksi & preferensi',
        ...
        FutureBuilder<PackageInfo>(
          ...
          builder: (context, snapshot) {
            ...
            return Container(
              ...
              child: Row(
                children: [
                  const Icon(Icons.info_outline, color: AppColors.gray, size: 16),
                  const SizedBox(width: 8),
                  Text('SeraphX v$version',
                      style: const TextStyle(
                          color: AppColors.gray, fontSize: 11, fontWeight: FontWeight.w600)),
                ],
              ),
            );
          },
        ...
        if (settings.aiProvider == AiProvider.customRest) ...[
          _block('Custom REST API URL', [
            ...
            const Padding(
              ...
              child: Text(
                'Buat API model GET (query param), pake placeholder {message}\n'
                'Contoh: https://api-kamu.com/ai?question={message}\n'
                'Kalau gak ada {message} di URL, otomatis pakai mode POST JSON.',
          ...
          _block('Nama Field Pesan (request, khusus mode POST)', [
            _field(_customRequestField, hint: 'message'),
            const Padding(
              padding: EdgeInsets.only(top: 6),
              child: Text(
                'Body yang dikirim: {"nama_field": "pesan user"}\n'
                '(gak dipakai kalau URL pakai mode GET/{message})',
                style: TextStyle(color: AppColors.gray, fontSize: 9.5),
              ),
            ),
          ]),
